    ...
    <div class="page-title">
      ...
      <el-row :gutter="10">
        <el-col :span="8">
            ...
            <span class="code" v-text="requestData.sheetNo">YSZJ0000000001</span>
          </div>
        </el-col>
        <el-col :span="8">
          <el-row type="flex" justify="center" align="middle">
            <span style="padding:0;" class="title" v-show="!showDraftTiTle">应收调账单</span>
            <span style="padding:0;" class="title" v-show="showDraftTiTle">应收调账单-草稿</span>
          </el-row>
        </el-col>
        <el-col :span="8">
    ...
    <div class="search-box-form">
      <el-row >
        <el-col :span="6" class="search-radio">
          <el-radio v-model="requestData.type" label="310" @change="radioCheck" >应收增加</el-radio>
          <el-radio v-model="requestData.type" label="320"  @change="radioCheck">应收减少</el-radio>
        </el-col>
        <el-col :span="6">
          <label>单据日期</label>
    ...
    <div class="table-contain">
      <el-table header-align="center" style="width: 100%;height:100%" height="100%" :data="tableData" border fit highlight-current-row size="mini" show-summary  :summary-method="getSummaries"> 
        ...
        <el-table-column prop="receivableMoney" label="金额" width="150">
          <template slot-scope="scope">
            <el-input ref="autoinput" size="mini" v-show="scope.row.editeFlage" v-model="scope.row.receivableMoney" @blur.stop="hiddenInputData(scope.row)"  :autofocus="true"></el-input>
...
import {fetchAdjustmentAddSheetNo,fetchAdjustmentLessenSheetNo,fetchReceiptAdjustment,fetchAdjustmentReceiptDarft,fetchAdjustmentReceiptDetail} from '@/api/receipt'
export default {
  ...
  methods:{
    ...
    darftUpdate(status){//草稿详情更新
      ...
      fetchAdjustmentReceiptDarft(this.requestData).then(response => {
        if(response.code===0){
          this.$message({
              type: 'success',
              message: '操作成功'
    ...
    addAdjustmentBill(status){//保存草稿、记账判断
        console.log(this.requestData.businessDate)
        if (this.requestData.businessDate === ''||this.requestData.businessDate === undefined) {
          this.tipShow("单据日期");
          return false
    ...
    postAdjustmentBill(status){ //保存草稿、记账提交后跳转历史      
     console.log(status)  
      ...
      this.requestData['sheetStatus']=status
      this.requestData['receivableMoney']=this.tableData[0].receivableMoney
      fetchReceiptAdjustment(this.requestData).then(response => {
        if(response.code===0){
          this.$message({
              type: 'success',
              message: '操作成功'
    ...
    getSheetNo(){//编号获取
      if(this.requestData.type==='310'){ //应收增加编号      
        fetchAdjustmentAddSheetNo().then(response => {   
          this.requestData.sheetNo=response.data.sheetNo
        })  
      }else{//应收减少编号
        fetchAdjustmentLessenSheetNo().then(response => {   
          this.requestData.sheetNo=response.data.sheetNo
        }) 
      }
    },
