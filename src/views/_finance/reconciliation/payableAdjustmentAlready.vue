      ...
      <el-row :gutter="10">
        ...
        <el-col :span="8">
          <el-row type="flex" justify="center" align="middle">
            <span style="padding:0;" class="title">应付调账单-已记账</span>
          </el-row>
        </el-col>
        <el-col :span="8">
    ...
    <div class="search-box-form">
      <el-row >
        <el-col :span="6" class="search-radio">
          <el-radio :disabled="true" v-model="requestData.type" label="330" >应付增加</el-radio>
          <el-radio :disabled="true" v-model="requestData.type" label="340">应付减少</el-radio>
        </el-col>
        <el-col :span="6">
          <label>单据日期</label>
...
<script>
import {fetchAdjustmentPayDisCard,fetchAdjustmentPayDetail} from '@/api/receipt'
export default {
  name: 'payadjustmentdiscard',
  components: {
  },
  data () {
    return {
  ...
  methods:{
    ...
    putAdjustmentDiscard(urlId){//应收调整作废
       fetchAdjustmentPayDisCard(urlId).then(response => {
        if(response.code===0){
          this.$message({
            type: 'success',
            message: '作废成功'
    ...
    getRouterDetailData(id){//草稿详情  
      fetchAdjustmentPayDetail(id).then(response => {
        if(response.code===0){
          ...
          _this.requestData.handlerUserId=response.data.receiptBill.handlerUserId;
