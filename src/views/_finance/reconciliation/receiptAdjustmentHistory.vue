  ...
  <div class="app-container receipt-container ">
      ...
      <el-row :gutter="10">
        ...
        <el-col :span="8">
          <el-row type="flex" justify="center" align="middle">
            <span style="padding:0;" class="title">应收调账-历史</span>
          </el-row>
        </el-col>
         <el-col :span="8">
    ...
    <div class="table-contain">
      <el-table  style="width: 100%;height:100%" height="100%" :data="tableData" 
        v-loading="loading" element-loading-text="数据加载中..." border fit highlight-current-row :stripe="true" size="mini">
        ...
        <el-table-column header-align="center" fixed label="操作" width="80px">
          <template slot-scope="scope">
            <div class="scope-flex">
            	<el-button v-if="scope.row.sheetStatus===40"  class="flex-btn" type="text" size="mini" @click="receiptDelete(scope.$index)">删除</el-button>
              <el-button v-else class="flex-btn" :disabled="true" type="text" size="mini"  >删除</el-button>
...
import {fetchReceiptBillPayDel,fetchAdjustmentReceiptHistory,fetchReceiptAdjustmentDel} from '@/api/receipt'
export default {
  name: 'receipthistory',
  data () {
    return {
  ...
  methods: {  
    ...
    toAdd(){//前往新增
        this.$router.push({path:'receivable_adjustment'})
    }, 
    ...
    queryReceiptList(){ //收款单历史列表
      ...
      fetchAdjustmentReceiptHistory(Data).then(response => {
    ...
    goHistory ({inOutbalanceId ,sheetStatus}) {//跳转相应单据详情
      let obj = {
        '40': 'receivable_adjustment',//草稿
        '10': 'receivable_adjustment_already',//已记账
        '20': 'receivable_adjustment_already',//已记账
        '30': 'receivable_adjustment_already'//已记账
      }
      if(obj[sheetStatus]==='receivable_adjustment'){
        this.$router.push({
    ...
    delReceiptHistory(receiptId, index) {
      const _this = this;
      fetchReceiptAdjustmentDel(receiptId).then(response => {
            if (response.code === 0) {
