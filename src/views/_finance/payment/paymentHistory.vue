<template>
  <div class="app-container receipt-container ">
    <div class="page-title">
      ...
      <el-row :gutter="10">
        <el-col :span="8">
        	<el-button type="danger" size="mini" align="left" icon="el-icon-circle-plus-outline">新增付款单</el-button>
        </el-col>
      </el-row>
      <div class="line"></div>
    </div>
    ...
    <div class="table-contain">
      ...
      :summary-method="getSummaries" show-summary v-loading="loading" element-loading-text="数据加载中..."
       border fit highlight-current-row :stripe="true" size="mini">
        ...
        <el-table-column header-align="center" fixed label="操作">
        ...
        <el-table-column header-align="center"  fixed prop="sheetDate" label="单据日期" width="95">
         
        </el-table-column>
        <el-table-column header-align="center" fixed prop="sheetNo" label="单据编号" width="190">
           <template slot-scope="scope">
        ...
        <el-table-column header-align="center" prop="actualMoney" label="金额"></el-table-column>
        <el-table-column header-align="center" prop="bankName1" width="120" label="收款账户1"></el-table-column>
        <el-table-column header-align="center" prop="paymentAmount1"  label="收款金额1">
          <template slot-scope="scope">
        ...
        <el-table-column header-align="center" prop="bankName2" width="120" label="收款账户2"></el-table-column>
        <el-table-column header-align="center" prop="paymentAmount2" label="收款金额2">
          <template slot-scope="scope">
        ...
        <el-table-column header-align="center" prop="" label="使用预收款" width="100"></el-table-column>
...
import {fetchReceiptBillPayDel,fetchQueryReceiptPayList,fetchReceivableAccount} from '@/api/receipt'
export default {
  name: 'paymenthistory',
  data () {
    return {
      loading:true,
      receivableaccountList:[],//收款账户列表
  ...
  methods: {
    ...
    goHistory ({settlementId ,sheetStatus}) {//跳转相应单据详情
      let obj = {
        '20': 'payment_sheet_draft',//草稿
        '10': 'payment_sheet_already'//已记账
      }
    ...
    handleSelect (item) {//下拉选择
      ...
      switch (this.which) {
        case 1:
          ...
          this.queryData.supplierName = item.value
          break
        case 3:
    ...
    receiptDelete (index) {
      const _this = this;
      if (_this.tableData[index].sheetStatus === 20) {
        _this.$confirm('此操作将删除该条订单, 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          _this.delReceiptHistory(_this.tableData[index].settlementId, index)
        }).catch(() => {
          _this.$message({
            type: 'info',
            message: '已取消删除'
    ...
    delReceiptHistory(receiptId, index) {
      const _this = this;
      fetchReceiptBillPayDel(receiptId).then(response => {
            if (response.code === 0) {
    ...
    queryReceiptList(){ //收款单历史列表
      ...
      this.loading=true
      fetchQueryReceiptPayList(Data).then(response => {
        if(response.code===0){
          this.loading=false
