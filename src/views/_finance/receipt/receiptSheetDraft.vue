      ...
      <el-row :gutter="10">
        ...
        <el-col :span="8">
          <el-row type="flex" justify="center" align="middle">
            <span style="padding:0;" class="title">收款单-草稿</span>
          </el-row>
        </el-col>
        <el-col :span="8">
    ...
    <div class="table-contain padding">
      <el-table style="width: 100%;height:calc100%" height="100%" :data="tableData" :summary-method="getSummaries"
        ...
        <el-table-column prop="settleMoney" label="本次结算">
          <template slot-scope="scope">              
             <el-input ref="recedarftinput" v-if="scope.row.receiptdarftFlage"   style="width:100%" size="mini" :autofocus="true" 
             ...
             <span ref="showtext" v-if="!scope.row.receiptdarftFlage"   @click="showInputData(scope.row,scope.$index)" style="width:100%;height:23px;display:inline-block;"   v-text="scope.row.settleMoney"></span>
          </template>
        </el-table-column>
...
// import tableEdit from '@/views/_finance/receipt/tableoperation'
import { mapGetters } from 'vuex';
import { fetchSheetCode, fetchGoodsList, fetchGoodsColor, fetchBrokerage, fetchSupplier, fetchSaveInstroeInfo, fetchSaveInstroeInfoDraft } from '@/api/purchase'
import {fetchReceivableAccountDetail,fetchReceivableAccount,fetchReceiptBillDetail,fetchReceiptBillUpdate} from '@/api/receipt'
...
export default {
  ...
  methods: {
    ...
    hiddenInputData(flag,index){//本次结算 编辑框隐藏
        this.$nextTick(() => {
          this.$set(flag, 'receiptdarftFlage', false)   
        })
    },
    showInputData(flag,index){//本次结算 编辑框显示
      if(flag.unSettleMoney!==""&&flag.unSettleMoney>0){
        this.$set(flag, 'receiptdarftFlage', true)   
        this.$nextTick(function(){ 
          this.$refs.recedarftinput.focus()                
        })
      }
    },
    ...
    postReceiptBill(status){  //保存草稿与记账
      ...
      fetchReceiptBillUpdate(data).then(response => {
        if(response.code===0){
          this.$message({
            type: 'success',
            message: '操作成功'
      ...
      if(_this.allChecked){
          ...
          if(_this.childrenCheckList!==null||_this.childrenCheckList!==[]){            
            ...
            _this.childrenCheckList.forEach(function(item){
                  _this.$set(item,'receiptdarftFlage',false)
    ...
    getBrokerage () {//获取经手人信息
      ...
      return new Promise((resolve, reject) => {
        fetchBrokerage().then(response => {
          _self.brokerage = response.data.data.list.list
          for (var i = 0; i < _self.brokerage.length; i++) {
            _self.brokerage[i].value = _self.brokerage[i].userName
          }
          resolve(true)
        }).catch(err => {
          console.log(err)
    ...
    getReceivableAccount() {//获取收款账户信息
      ...
      return new Promise((resolve, reject) => {
        fetchReceivableAccount().then(response => {
          _self.receivableaccount = response.data.list.list
          for (var i = 0; i < _self.receivableaccount.length; i++) {
              _self.receivableaccount[i].value =_self.receivableaccount[i].bankName
          }
           _self.receivableaccount2 = response.data.list.list
          for (var i = 0; i < _self.receivableaccount2.length; i++) {
              _self.receivableaccount2[i].value = _self.receivableaccount2[i].bankName
          }
          resolve(true)
        }).catch(err => {
          console.log(err)
    ...
    getSupplier () {//客户列表
      ...
      return new Promise((resolve, reject) => {
        fetchSupplier().then(response => {
          _self.supplier = response.data.data.list.list
          for (var i = 0; i < _self.supplier.length; i++) {
            _self.supplier[i].value = _self.supplier[i].customerName
          }
          resolve(true)
        }).catch(err => {
          console.log(err)
    ...
    getTableData () {
      for (var i = 0; i < 10; i++) {
        this.tableData.push({ businessDate: '', businessNo: '', saleMoney: '', unSettleMoney: '', settleMoney: '', sheetHandler: ''})
      }
    },
