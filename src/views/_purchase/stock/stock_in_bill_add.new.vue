<template>
  ...
  <div class="app-container stockinbill-container">
    <page-header :print="print" title="采购入库单" :orderNumber="requestData.orderNo" :historyUrl="'/'"></page-header>
    <search-box :searchOption="searchOption" :comChange="searchOnChange" ref="search"></search-box>
    <!-- :query="query" -->
...
export default {
  ...
  methods: {
    ...
    changeSelectData () {
      ...
      this.trConfig[1].select_data = this.commodityArr //编号
    },
    refresh () {
      console.log('刷新')
    },
    ...
    submit (ev, key) {
      this.$refs.search.submitForm(() => {
        saveInstroeInfo(this.requestData).then(response => {
          this.$message({
            type: 'success',
            message: '保存成功'
          })
    ...
    draft () {
      this.$refs.search.submitForm(() => {
        saveInstroeInfoDraft(this.requestData).then(response => {
          this.$message({
            type: 'success',
            message: '保存成功'
          })
...
<style>
  .stockinbill-container .inputTable{
    bottom: 17px;
    margin-top: 41px;
    margin-bottom: 60px;
  }
