<template>
	<div class="app-container" v-loading="loading">
		<div class="page-title">
			...
			<el-row>
				<el-col :span="8">
					...
					<span class="code-no">{{queryData.stockAllotNo}}</span>
				</el-col>
				<el-col :span="8">
		...
		<div class="term-search">
			<el-row>
				<el-col :span="5">
					...
					<el-autocomplete v-model="queryData.storeName" @focus="which=1" class="input-item" :fetch-suggestions="querySearchs" placeholder="选择仓库" @select="handleSelect">
				...
				<el-col :span="5">
					...
					<el-autocomplete v-model="queryData.targetStoreName" @focus="which=2" class="input-item" :fetch-suggestions="querySearchs" placeholder="选择仓库" @select="handleSelect">
				...
				<el-col :span="5">
					...
					<el-autocomplete v-model="queryData.handler" @focus="which=3" class="input-item" :fetch-suggestions="querySearchs" placeholder="输入经手人" @select="handleSelect" clearable>
			...
			<el-table height="100%" style="width: 100%;" :data="tableData" border fit highlight-current-row :summary-method="getSummaries" show-summary>
				...
				<el-table-column prop="goodsNo" label="编号">
					<template slot-scope="scope">
						...
						<div class="inner-text" style="width:100%;height:28px; line-height=28px; position:relative;" v-if="tableData[scope.$index].Nochange==false" @click.shop="checkstore(),showinput(scope.$index,'goodsNoShow','NoinnerInput')">
							{{scope.row.goodsNo}}
							<el-autocomplete v-model="value" v-if="tableData[scope.$index].goodsNoShow" @blur="hideinput(scope.$index,'goodsNoShow')" size="mini" @focus="which=1" @select="gogo(scope.$index,scope.row,'goodsNoShow')" :trigger-on-focus="false" :fetch-suggestions="queryTable" style="position:absolute;left:0;top:0;" ref="NoinnerInput">
							</el-autocomplete>
						</div>
					</template>
				</el-table-column>
				<el-table-column prop="goodsName" label="品名">
					<template slot-scope="scope">
						...
						<div class="inner-text" style="width:100%;height:28px; line-height=28px; position:relative;" v-if="tableData[scope.$index].Nochange==false" @click.shop="checkstore(),showinput(scope.$index,'goodsNameShow','NameinnerInput')">
							{{scope.row.goodsName}}
							<el-autocomplete
								v-model="value"
				...
				<el-table-column prop="goodsColorNo" label="颜色">
					<template slot-scope="scope">
						...
						<div class="inner-text" style="width:100%;height:28px; line-height=28px; position:relative;" @click.shop="showinput(scope.$index,'colorSelectFade','ColorinnerInput')" v-if="tableData[scope.$index].colorSelectBoxFade">
							{{scope.row.goodsColorNo}}
							<el-select v-model="scope.row.goodsColorNo" placeholder="选择颜色" filterable remote size="mini" :remote-method="ColorremoteMethod" :loding="selectloading" v-if="tableData[scope.$index].colorSelectFade" ref="ColorinnerInput" style="position:absolute;left:0;top:0;" @blur="hideinput(scope.$index,'colorSelectFade')" @change="getGoodsbatchNo(scope.row.goodsColorNo,scope.row.goodsNo,scope.$index)">
								<el-option v-for="item in coloroption" :key="item.valuekey" :label="item.label" :value="item.value">
								</el-option>
							</el-select>
						</div>
					</template>
		        ...
		        <el-table-column prop="optItemQuantity" label="调出条数">
			        	<template slot-scope="scope">
			        		<div class="inner-text" style="width:100%;height:28px; line-height=28px; position:relative;" v-if="tableData[scope.$index].lastInputFade" @click.shop="showinput(scope.$index,'optItemQuantityinputFade','optItemQuantityinput')">
			        			{{scope.row.optItemQuantity}}
			        			<el-input style="position:absolute;left:0;top:0;" v-if="tableData[scope.$index].optItemQuantityinputFade" v-model="scope.row.optItemQuantity" ref='optItemQuantityinput' @blur="hideinput(scope.$index,'optItemQuantityinputFade')" size="mini"></el-input>
			        		</div>
			        	</template>
		        </el-table-column>
				<el-table-column prop="optQuantity" label="调出数量">
					<template slot-scope="scope">
						<div class="inner-text" style="width:100%;height:28px; line-height=28px; position:relative;" v-if="tableData[scope.$index].lastInputFade" @click.shop="showinput(scope.$index,'optQuantityinputFade','optQuantityinput')">
							{{scope.row.optQuantity}}
							<el-input style="position:absolute;left:0;top:0;" v-if="tableData[scope.$index].optQuantityinputFade" v-model="scope.row.optQuantity" ref='optQuantityinput' @blur="hideinput(scope.$index,'optQuantityinputFade')" size="mini"></el-input>
						</div>
					</template>
				</el-table-column>
				<!--<el-table-column prop="optQuantity" label="调拨单价"></el-table-column>
				<el-table-column prop="optQuantity" label="调拨金额"></el-table-column>-->
		...
		<div class="btn-contain">
			...
			<el-button size="mini" type="primary" @click="save()">申请调拨</el-button>
		</div>
...
<script>
import { AddstockCount,getStockCheckNo,fenchqueryStock,fetchStockList,AddstockCountDraft,getstockAllotNo,fetchAddStockAllot,fetchgoodsList,fetchgoodsColorList} from "@/api/inventory";
...
//				表格数据
				tableData:[{
					...
					lastInputFade:false,//最后的input输入框显示
					optItemQuantityinputFade:false,//调出条数输入
					optQuantityinputFade:false,//调出数量输入
...
//			          this.loading = false;
		    ...
		    getTableInfo (cbdata,i) {
					...
					this.$set(item,'optItemQuantityinputFade',false);
					this.$set(item,'optQuantityinputFade',false);
		    		...
		    		console.log('列属性',this.tableData);
...
//			获取单据编号接口
			getStockCheckNo(){
				const _self = this;
				return new Promise((resolve, reject) => {
					getstockAllotNo().then(response=>{
						if (response.code === 0) {
							_self.queryData.stockAllotNo = response.data.stockAllotNo;
							resolve(true);
						}else{
							this.$message({
			                		type: "error",
...
//		    },
			...
			handleSelect(item){
		        ...
		        case 2:
		          ...
		          this.queryData.targetStoreNo = item.storeNo;
		          break;
		        case 3:
	      	...
	      	getSummaries(param) {
		        ...
		        columns.forEach((column, index) => {
			          ...
			          if(index == 1||index == 2||index == 3||index == 4||index == 5||index == 6||index == 11||index == 12){
			          	sums[index] = '';
			            return;
			          }
...
//	      弹层方法
		    ...
		    targetStoreInfo (data) {
		      ...
		      this.queryData.targetStoreNo = data.storeNo
		    },
		    getHandlerInfo (data) {
		   ...
		   getDeport(){
		    ...
		    tranfer (data) {
		       this.queryData.stockAllotDetailList = []
		       data.forEach((item,index)=>{
		       		let obj = {
//		       			'storeLocationId':item.storeLocationId,
		       			'goodsId':item.stockId,
		       			...
		       			'optItemQuantity':item.optItemQuantity,
		       			'optQuantity':item.optQuantity,
		       			'goodsUnit':item.stockUnit,
		       			'remark':item.remark,
		       		}
		       		this.queryData.stockAllotDetailList.push(obj);
		       })
		    },		  
		    //保存盘点单
		    save(){
			    ...
			    if (this.queryData.storeName === '') {
		      		...
		      		this.$message({
		        		...
		        		return false
		      	}else if(this.queryData.stockAllotDetailList.length === 0){
		      		this.$message({
			        		type: 'error',
			        		message: '盘点商品内容不能为空'
		        		})	
		      		...
		      		this.queryData.totalItem = 0;
		      		this.queryData.stockAllotDetailList.forEach((item,index)=>{
		      			this.queryData.totalItem += Number(item.optItemQuantity);
		      		});
		      		this.queryData.status = 10;
		      		return new Promise((resolve, reject) => {
		      			fetchAddStockAllot(this.queryData).then(response=>{
