  ...
  <div class="app-container" v-loading="loading">
    ...
    <div class="term-searcher">
    		...
    		<el-row>
    			...
    			<el-col :span="6">
    				...
    				<el-checkbox v-model="checked" size="mini" @change="showCancel">显示作废</el-checkbox>
    			</el-col>
    		</el-row>
    	</div>
    ...
    <div class="table-contain">
      <el-table style="width: 100%;height:100%" height="100%" :data="tableData" border fit highlight-current-row>
        ...
        <el-table-column label="操作" width="120">
          <template slot-scope="scope">
            <div class="scope-flex">
              <el-button class="flex-btn" type="text" size="mini" :disabled="scope.row.status!=10? true:false" @click="confirm(scope.$index)">审核</el-button>
              <el-button class="flex-btn" type="text" size="mini" @click="goHistory(scope.row)">详情</el-button>
              <el-button class="flex-btn" :disabled="scope.row.status===20? false:true" type="text" size="mini" @click="deleteList(scope.$index)">删除</el-button>
        ...
        <el-table-column prop="stockAllotNo" label="单据编号" width="190">
          <template slot-scope="scope">
            <span class="blue1" @click="goHistory(scope.row)" style="cursor:pointer;" v-text="scope.row.stockAllotNo"></span>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="storeName" label="调出仓库"></el-table-column>
        <el-table-column prop="targetStoreName" label="调入仓库"></el-table-column>
        <el-table-column prop="totalItem" label="调拨总条数"></el-table-column>
        <el-table-column prop="totalQuantity" label="调拨总数量"></el-table-column>
...
import { stockAllotList,DelectAllot,stockAllotConfirm } from "@/api/inventory";
...
export default {
  name: "inventoryAllocationHistory",
  components: {
//  SearchBox
	fixedPage2,
	fixedPage3
...
//    ]
  ...
  methods: {
    ...
    handleCurrentChange(val) {
      ...
      this.fetchData(this.queryData);
      console.log(2,this.queryData.pageSize);
    },
    searchOnChange(key, data) {
      this.queryData[key] = data;
    },
    goHistory({ status, stockAllotId }) {
      let obj = {
        "-10": "inventory_allocationCancel", //已作废
        "10": "inventory_allocationReview", //待审核
        "20": "inventory_allocationDraft", //草稿
        "30": "inventory_allocationAlready" //已完成
      };
      this.$router.push({
        name: obj[status],
        query: { stockAllotId }
...
//  		const _self = this;
//  		let fromdate,todate;
//  		if(_self.queryData.sheetDate){
//  				fromdate=_self.queryData.sheetDate[0];
//  				todate=_self.queryData.sheetDate[1];
//  			}else{
//  				fromdate='';
//  				todate='';
//  			}
...
//  		});
    ...
    newBill(){
    		this.$router.push('inventory_allocation')
    },
    //删除表单
    deleteList(i){
    		const _self =this;
    		return new Promise((resolve, reject)=>{
    			DelectAllot(_self.tableData[i].stockAllotId).then(response=>{
    				this.$message({
    					message:'删除成功',
    					type:'success'
    				})
	...
	confirm(i){
			const _self = this
			return new Promise((resolve,reject)=>{
				stockAllotConfirm(_self.tableData[i].stockAllotId).then(response=>{
					this.$message({
						message:'审核成功通过',
						type:'success'
					})
	...
	showCancel(){
		if(!this.checked){
			var CancelList = this.tableData.filter((item)=>{
				return item.status != -10
			})
			this.tableData = CancelList;
		}else{
			this.fetchData(this.queryData);
		}
	}
