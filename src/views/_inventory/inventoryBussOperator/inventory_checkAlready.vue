	...
	<div class="app-container" v-loading="loading">
		<div class="page-title">
			...
			<el-row>
				<el-col :span="8">
					...
					<span class="code-no">{{option.stockCountNo}}</span>
				</el-col>
				<el-col :span="8">
		...
		<div class="term-search">
			<el-row>
				<el-col :span="5">
					...
					<el-autocomplete v-model="queryData.storeName" @focus="which=1" class="input-item" :fetch-suggestions="querySearchs" placeholder="选择仓库" @select="handleSelect" clearable disabled>
				...
				<el-col :span="5">
					...
					<el-autocomplete v-model="queryData.handler" @focus="which=3" class="input-item" :fetch-suggestions="querySearchs" placeholder="输入经手人" @select="handleSelect" clearable disabled>
						<el-button slot="append" icon="el-icon-search" @click="fixedOption.brokerageShow=true"></el-button>
					</el-autocomplete>
				</el-col>
				<el-col :span="4">
					<span class="serch-title">盘点类型</span>
					<el-select v-model="queryData.countType" class="input-item" placeholder="请选择" @change="hh()" disabled>
					    <el-option
				...
				<el-col :span="5">
					...
					<el-date-picker v-model="queryData.sheetDate" type="date" class="input-item" value-format="yyyy-MM-dd" placeholder=" 单据日期(止)" disabled>
					</el-date-picker>
				</el-col>
				<el-col :span="2">
		...
		<div class="table-contain">
			<el-table height="100%" style="width: 100%;" :data="tableData" border fit highlight-current-row :summary-method="getSummaries" show-summary>
				...
				<!--<el-table-column prop="optItemQuantity" label="库存条数"></el-table-column>-->
				<el-table-column prop="beforeItemQuantity" label="库存数量"></el-table-column>
				<el-table-column prop="afterGoodsUnit" label="单位"></el-table-column>
				<!--<el-table-column label="库存条数"></el-table-column>
		        <el-table-column label="库存数量"></el-table-column>-->
				<el-table-column prop="afterItemQuantity" label="盘点后条数"></el-table-column>
				<el-table-column prop="afterQuantity" label="盘点后数量"></el-table-column>
				<el-table-column label="盘点后状态">
					<template slot-scope="scope">
...
<script>
import { fenchstockCountContent } from "@/api/inventory";
...
//				表格数据
		...
		methods:{
			hh(val){console.log(val)},
			goHistory(){
				this.$router.push('inventory_check_history')
			},
//			获取表格数据
			getReceiptDraftList(){
				let urlId=this.$route.query.stockCountId;
				const _self = this;
				return new Promise((resolve, reject) => {
					fenchstockCountContent(urlId).then(response=>{
						if (response.code === 0) {
							console.log(response);
							let data = response.data.stockCount;
							_self.queryData.handler = data.handler;
							_self.queryData.countType = data.countType;
...
//							_self.queryData.targetStoreName = data.targetStoreName;
							_self.option.stockCountNo = data.stockCountNo;
							_self.tableData = data.stockCountDetailList;
							resolve(true);
						}else{
							this.$message({
			                		type: "error",
			...
			handleSelect(item){
	      	...
	      	getSummaries(param) {
		        ...
		        columns.forEach((column, index) => {
			          ...
			          if(index == 1||index == 2||index == 3||index == 4||index == 5||index == 7||index == 9||index == 10){
			          	sums[index] = '';
			            return;
			          }
