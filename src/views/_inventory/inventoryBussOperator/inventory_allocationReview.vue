<template>
	<div class="app-container" v-loading="loading">
			...
			<el-row>
				<el-col :span="5">
					...
					<el-autocomplete disabled v-model="queryData.storeName" @focus="which=1" class="input-item" :fetch-suggestions="querySearchs" placeholder="选择仓库" @select="handleSelect" clearable>
						<!--<el-button disabled slot="append" icon="el-icon-search" @click="fixedOption.deportShow=true,StoreNameFade=true"></el-button>-->
					</el-autocomplete>
				</el-col>
				<el-col :span="5">
		...
		<div class="remark-contain">
			<el-input clearable disabled v-model="queryData.remark" placeholder="单据备注">
			</el-input>
		</div>
		<div class="btn-contain">
			<el-button size="mini" @click="cancel">取消</el-button>
			<el-button type="primary" size="mini" @click="confirm">审核入库</el-button>
		</div>
...
<script>
import { fetchstockAllotContent,stockAllotConfirm,stockAllotcancel } from "@/api/inventory";
...
//	      弹层方法
		   ...
		   getDeport(){
		    ...
		    confirm(){
		    		const _self = this
		    		return new Promise((resolve,reject)=>{
		    			stockAllotConfirm(_self.queryData.stockAllotId).then(response=>{
		    				this.$message({
		    					message:'审核成功通过',
		    					type:'success'
		    				})
		    ...
		    cancel(){
		    		const _self = this
		    		return new Promise((resolve,reject)=>{
		    			stockAllotcancel(_self.queryData.stockAllotId).then(response=>{
		    				this.$message({
		    					message:'取消审核成功',
		    					type:'success'
		    				})
