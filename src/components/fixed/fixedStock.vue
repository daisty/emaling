<template>
  <transition name="fade-in">
    <div class="fix-box" :style="zIndex" v-show="fixedOption.stockShow">
      <div class="content" :style="wh" v-show="fixedOption.stockShow">
        <div class="fix-top" :style="backgroundColor">
          <span class="text">库存选择</span>
          <b class="close" @click="fixedOption.stockShow=false">×</b>
        </div>
        <div class="fix-content">
          <div class="search">
            <div class="input">
              <el-input size="mini" v-model="goodsNo" placeholder="请输入商品编号查询"></el-input>
            </div>
...
<script>
  ...
  import {fetchStockList} from '@/api/sale'
    ...
    methods: {
      ...
      getDeport() {
        ...
        return new Promise((resolve, reject) => {
          fetchStockList({pageNum: this.pageNum, pageSize: this.pageSize, goodsNo: this.goodsNo})
            .then(response => {
      ...
      sureChose() {
        ...
        this.$emit('stockInfo', {
          goodsNo: this.tableData.list[this.index].goodsNo,
          stockId: this.tableData.list[this.index].stockId
        })
        this.fixedOption.stockShow = false
      },
      cancle() {
        this.index = ''
        this.fixedOption.stockShow = false
