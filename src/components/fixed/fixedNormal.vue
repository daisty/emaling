<template>
  <transition name="fade-in">
    <div class="fix-box" :style="zIndex" v-show="fixedOption.deportShow">
      <div class="content" :style="wh" v-show="fixedOption.deportShow">
        <div class="fix-top" :style="backgroundColor">
          <span class="text">仓库选择</span>
          <b class="close" @click="fixedOption.deportShow=false">×</b>
        </div>
        <div class="fix-content">
          <div class="search">
            <div class="input">
              <el-input size="mini" v-model="storeName" placeholder="请输入仓库名称/编号查询"></el-input>
            </div>
            <el-button size="mini" @click="getDeport()">查询</el-button>
            <el-button style="float:right" size="mini" @click="eve"><i class="el-icon-plus"></i>新增</el-button>
          </div>
          <deport-table @someInfo="getInfo" :tableData="tableData.list"></deport-table>
          <this-url></this-url>
          <div class="page-contain">
            ...
            <el-button style="float:right;margin-right:10px;" type="primary" size="mini" @click="sureChose()">
              确定(Enter)
            </el-button>
          </div>
        </div>
...
<script>
  import deportTable from './components/deportTable'
    ...
    methods: {
      ...
      getDeport() {
        return new Promise((resolve, reject) => {
          fetchDeport({pageNum: this.pageNum, pageSize: this.pageSize, storeNo: this.storeName})
            .then(response => {
              var res = response.data
              if (res.code === 0) {
                this.tableData = res.data.storeInfoList
                this.pages = res.data.storeInfoList.pages
                this.pageNum = res.data.storeInfoList.pageNum
                this.pageSize = res.data.storeInfoList.pageSize
                this.total = res.data.storeInfoList.total
                this.jumperPage = res.data.storeInfoList.pageNum
              } else {
                this.$message({
                  type: 'error',
                  message: res.msg
      ...
      pageChange(index) {
        if (index === 1) {
          ...
          if (this.jumperPage === '' || this.jumperPage < 1 || this.jumperPage > this.pages) {
        ...
        this.getDeport({pageNum: this.pageNum, pageSize: this.pageSize})
      },
      sureChose() {
        if (this.index === '') {
          this.$message({
        ...
        this.$emit('deportInfo', {
          name: this.tableData.list[this.index].storeName,
          id: this.tableData.list[this.index].storeId
        })
        this.fixedOption.deportShow = false
      },
      cancle() {
        this.index = ''
        this.fixedOption.deportShow = false
