<template>
  <transition name="fade-in">  
    <div class="fix-box" :style="zIndex" v-show="fixedOption.supplierShow">
      <div class="content" :style="wh" v-show="fixedOption.supplierShow">
        <div class="fix-top" :style="backgroundColor">
          <span class="text">供应商选择</span>
          <b class="close" @click="fixedOption.supplierShow=false">×</b>
        </div>
        <div class="fix-content">
          <div class="search">
            <div class="input">
              <el-input size="mini" v-model="customerName" placeholder="请输入供应商名称/编号查询"></el-input>
            </div>
            <el-button size="mini" @click="getSupplier()">查询</el-button>
            <el-button style="float:right" size="mini" @click="eve"><i class="el-icon-plus"></i>新增</el-button>
          </div>
          <supplier-table @someInfo="getInfo" :tableData="tableData.list"></supplier-table>
          <this-url></this-url>
...
import { fetchSupplier } from "@/api/purchase";
export default {
  props: {
    fixedOption: {
      Object
  ...
  methods: {
    ...
    pageChange(index) {
      ...
      this.getSupplier({ pageNum: this.pageNum, pageSize: this.pageSize });
    },
    sureChose() {
      if (this.index === "") {
        this.$message({
      ...
      this.$emit("supplierInfo", {
        name: this.tableData.list[this.index].customerName,
        id: this.tableData.list[this.index].customerId
      });
      this.fixedOption.supplierShow = false;
    },
    cancle() {
      this.index = "";
      this.fixedOption.supplierShow = false;
